...
async function request<T>(path: string, apiKey: string, params?: Record<string, string | number | undefined>): Promise<T> {
  ...
  if (!response.ok) {
    ...
    try { const parsed = JSON.parse(body); detail = parsed?.message || parsed?.error || parsed?.detail || detail; } catch {}
...
export async function listAssets(apiKey: string, ids?: string[]) {
  ...
  for (const id of unique) { const page = await request<Page<BitpandaAsset>>("/assets", apiKey, { id }); chunks.push(...(page.data || [])); }
  return chunks;
}
