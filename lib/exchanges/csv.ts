...
function asDate(value: unknown) {
  const raw = clean(value); if (!raw) return null; const n = Number(raw);
...
function parseLine(line: string, delimiter: string) {
  ...
  for (let i = 0; i < line.length; i += 1) { const ch = line[i]; if (ch === '"') { if (quoted && line[i + 1] === '"') { cell += '"'; i += 1; } else quoted = !quoted; } else if (ch === delimiter && !quoted) { out.push(cell); cell = ""; } else cell += ch; }
...
function readRows(text: string) {
  ...
  const rows: CsvRow[] = [];
  for (let i = headerInfo.index + 1; i < lines.length; i += 1) { if (!lines[i].trim()) continue; const cells = parseLine(lines[i], delimiter); const row: CsvRow = {}; headers.forEach((h, j) => { row[h] = clean(cells[j]); }); if (Object.values(row).some(Boolean)) rows.push(row); }
  return rows;
}
function rowMap(row: CsvRow) { const values = new Map(Object.entries(row).map(([k, v]) => [headerKey(k), clean(v)])); return (...names: string[]) => { for (const name of names) { const value = values.get(headerKey(name)); if (value) return value; } return ""; }; }
function pairAssets(pair: string, base: string, quote: string) { const b = asAsset(base), q = asAsset(quote); if (b && q) return { base: b, quote: q }; const p = clean(pair).replace(/[\s_\/-]/g, "").toUpperCase(); const candidate = QUOTES.find((x) => p.endsWith(x) && p.length > x.length); return candidate ? { base: p.slice(0, -candidate.length), quote: candidate } : { base: b, quote: q }; }
function classify(type: string, direction = "", description = "") {
  const value = `${type} ${description}`.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[-\s]+/g, "_"); const flow = direction.toLowerCase();
  if (/buy|purchase|bought|compra|comprado|comprada/.test(value)) return "buy"; if (/sell|sold|venta|vendido|vendida/.test(value)) return "sell"; if (/convert|conversion|swap|convertir/.test(value)) return "trade";
  if (/withdraw|cash_out|removal|entnahme|retirada/.test(value)) return "withdrawal"; if (/deposit|receive|received|cash_in|deposito|entrada/.test(value)) return "deposit";
  if (/transfer/.test(value)) return flow.includes("out") ? "transfer_out" : flow.includes("in") ? "transfer_in" : "transfer"; if (/staking|stake|earn/.test(value)) return "staking";
  if (/reward|bonus|referral/.test(value)) return "reward"; if (/airdrop|fork/.test(value)) return "airdrop"; if (/interest|yield|interes|interés/.test(value)) return "interest";
  if (/cashback/.test(value)) return "cashback"; if (/fee|commission|comision|comisión|fees/.test(value)) return "fee"; if (/dividend|dividendo/.test(value)) return "income";
  if (/refund|reversal|reembolso/.test(value)) return "income"; if (/card.*payment|payment.*card|pago.*tarjeta/.test(value)) return "expense";
  if (/out|debit|salida/.test(flow) || flow === "-") return "withdrawal"; if (/in|credit|entrada/.test(flow) || flow === "+") return "deposit"; return "other";
}
function signed(value: number | null, type: string, direction: string) {
  if (value === null) return null; if (["buy", "deposit", "transfer_in", "reward", "staking", "airdrop", "interest", "cashback", "income"].includes(type)) return Math.abs(value);
  if (["sell", "withdrawal", "transfer_out", "fee", "expense"].includes(type)) return -Math.abs(value); if (/out|debit|salida/.test(direction.toLowerCase())) return -Math.abs(value); if (/in|credit|entrada/.test(direction.toLowerCase())) return Math.abs(value); return value;
}
function movementDirection(amount: number | null, direction: string) { const f = direction.toLowerCase(); if (/out|debit|salida/.test(f) || (amount !== null && amount < 0)) return "outgoing" as const; if (/in|credit|entrada/.test(f) || (amount !== null && amount > 0)) return "incoming" as const; return "neutral" as const; }
...
function bitpanda(rows: CsvRow[], file: string) {
  return rows.map((row) => {
    const get = rowMap(row); const original = get("Transaction Type", "Type", "Operation Type") || "unknown"; const direction = get("In/Out", "Direction"); const type = classify(original, direction);
    const asset = asAsset(get("Asset", "Cryptocurrency")); const fiat = asAsset(get("Fiat", "Currency")) || "EUR";
    const assetAmount = asNumber(get("Amount Asset", "Asset Amount", "Shares", "Quantity")); const fiatAmount = asNumber(get("Amount Fiat", "Fiat Amount", "Total", "Amount EUR"));
    const fee = asNumber(get("Fee", "Fees", "Fee Amount")); const feeAsset = asAsset(get("Fee Asset", "Fee Currency")); const price = asNumber(get("Asset Market Price", "Price", "Rate")); const priceCurrency = asAsset(get("Asset Market Price Currency", "Price Currency", "Currency")) || fiat;
    const isTrade = type === "buy" || type === "sell";
    const baseAsset = isTrade ? asset : (assetAmount !== null && asset ? asset : fiat);
    const baseAmount = isTrade ? (assetAmount === null ? null : type === "buy" ? Math.abs(assetAmount) : -Math.abs(assetAmount)) : assetAmount !== null && asset ? signed(assetAmount, type, direction) : signed(fiatAmount, type, direction);
...
function kraken(rows: CsvRow[], file: string) {
  return rows.map((row) => { const get = rowMap(row); const original = get("Type", "Transaction Type") || "unknown"; const subtype = get("Subtype", "Sub Type"); const direction = get("Direction", "Flow"); const pair = pairAssets(get("Pair", "Symbol"), get("Base Asset"), get("Quote Asset")); const amount = asNumber(get("Amount", "Vol", "Volume", "Quantity")); const cost = asNumber(get("Cost", "Total", "Funds")); const fee = asNumber(get("Fee", "Fees")); const asset = asAsset(get("Asset", "Currency")) || pair.base; const side = get("Side"); const type = original.toLowerCase() === "trade" ? (side.toLowerCase() === "sell" ? "sell" : "buy") : classify(original, direction, subtype); let baseAsset = asset; let baseAmount = signed(amount, type, direction); let quoteAmount: number | null = null; if (original.toLowerCase() === "trade" && pair.base && pair.quote && amount !== null && cost !== null) { baseAsset = pair.base; baseAmount = side.toLowerCase() === "sell" ? -Math.abs(amount) : Math.abs(amount); quoteAmount = side.toLowerCase() === "sell" ? Math.abs(cost) : -Math.abs(cost); } const parser = original.toLowerCase() === "trade" ? "kraken-trades" : "kraken-ledger"; const m = make("kraken", parser, row, { occurredAt: asDate(get("Time", "Timestamp", "Date")) || "", transactionType: type, originalType: subtype ? `${original} / ${subtype}` : original, direction, baseAsset, baseAmount, quoteAsset: quoteAmount !== null ? pair.quote : null, quoteAmount, feeAsset: fee !== null ? asset : null, feeAmount: fee === null ? null : -Math.abs(fee), price: asNumber(get("Price", "Rate")), priceCurrency: pair.quote }); m.externalId = stableId("kraken", parser, row, get("Txid", "TXID", "Transaction ID", "Refid", "Reference ID", "Ordertxid", "Trade ID", "ID")); m.raw.sourceFile = file; return m; });
}

function generic(exchange: string, rows: CsvRow[], file: string) {
  return rows.map((row) => { const get = rowMap(row); const original = get("Transaction Type", "Type", "Business Type", "Sub Type", "Category", "Activity", "Operation", "Transaction Description", "Description") || "unknown"; const direction = get("Direction", "Cash Flow", "Flow", "In/Out", "Side"); const description = get("Transaction Description", "Description", "Remark", "Notes"); const type = classify(original, direction, description); const pair = pairAssets(get("Pair", "Symbol", "Instrument"), get("Base Asset", "Base Coin"), get("Quote Asset", "Quote Coin")); const coin = asAsset(get("Coin", "Asset", "Currency")) || pair.base; const amount = asNumber(get("Change", "Net Change", "Amount", "Quantity", "Volume", "Balance Change")); const total = asNumber(get("Total", "Quote Amount", "Cost", "Funds", "Counter Amount")); const fee = asNumber(get("Fee", "Fees", "Commission", "Transaction Fee")); const feeAsset = asAsset(get("Fee Asset", "Fee Currency", "Commission Asset")); const price = asNumber(get("Price", "Average Price", "Execution Price", "Rate")); let baseAmount = signed(amount, type, direction); let quoteAsset: string | null = null; let quoteAmount: number | null = null; if (pair.base && pair.quote && amount !== null && total !== null && (type === "buy" || type === "sell")) { baseAmount = type === "buy" ? Math.abs(amount) : -Math.abs(amount); quoteAsset = pair.quote; quoteAmount = type === "buy" ? -Math.abs(total) : Math.abs(total); } const parser = `${exchange}-generic`; const m = make(exchange, parser, row, { occurredAt: asDate(get("Timestamp", "Time", "Date", "Created At", "UTC Time", "Date(UTC)")) || "", transactionType: type, originalType: original, direction, baseAsset: coin, baseAmount, quoteAsset, quoteAmount, feeAsset, feeAmount: fee === null ? null : -Math.abs(fee), price, priceCurrency: asAsset(get("Price Currency", "Quote Currency", "Currency")) || pair.quote }); m.externalId = stableId(exchange, parser, row, get("Transaction ID", "TransactionID", "TXID", "TxID", "Trade ID", "Order ID", "OrderId", "ID", "Hash", "Reference")); m.raw.sourceFile = file; return m; });
}

export function normalizeExchangeCsv(exchange: string, rows: CsvRow[], fileName = "") {
  const code = exchange.trim().toLowerCase();
  const movements = code === "bitpanda" ? bitpanda(rows, fileName) : code === "binance" ? binance(rows, fileName) : code === "coinbase" ? coinbase(rows, fileName) : code === "kraken" ? kraken(rows, fileName) : CSV_SUPPORTED_EXCHANGES.has(code) ? generic(code, rows, fileName) : (() => { throw new Error(`Exchange no soportado: ${exchange}`); })();
