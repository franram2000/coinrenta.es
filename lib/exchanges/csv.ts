...
function asDate(value: unknown) {
  ...
  if (Number.isFinite(n) && raw.length >= 10) {
    const date = new Date(n < 2_000_000_000 ? n * 1000 : n);
...
function readRows(text: string) {
  ...
  const headerInfo = nonEmpty.find((item) => {
    ...
    return key.includes("transactionid") || key.includes("userid") || key.includes("utc_time") || key.includes("utctime") ||
      key.includes("timestamp") || key.includes("transactiontype") || key.includes("businesstype") || key.includes("ordertype") ||
      key.includes("subtype") || key.includes("transtype") || key.includes("biztype");
  }) || nonEmpty[0];
...
function classify(type: string, direction = "", description = "") {
  ...
  if (/convert|conversion|swap|convertir/.test(value)) return "trade";
...
function make(exchange: string, parser: string, row: CsvRow, input: MovementInput): NormalizedMovement {
  ...
  return {
    ...
    direction: movementDirection(input.baseAmount, rawDirection),
    classification: input.transactionType === "other" ? "needs_review" : "classified",
...
function bitpanda(rows: CsvRow[], file: string) {
  return rows.map((row) => {
    ...
    // Bitpanda history has two independent amount concepts: Amount Fiat is the fiat leg/value,
    // while Amount Asset is the actual asset quantity. Never treat a fiat valuation as an ETH/BTC quantity.
    const baseAsset = trade ? asset : assetAmount !== null && asset ? asset : fiat;
    const baseAmount = trade
    ...
    const quoteAmount = trade && fiatAmount !== null ? (type === "buy" ? -Math.abs(fiatAmount) : Math.abs(fiatAmount)) : null;

    const parser = "bitpanda-history";
    const movement = make("bitpanda", parser, row, {
...
function binance(rows: CsvRow[], file: string) {
  return rows.map((row) => {
    const get = rowMap(row);
    const utc = get("UTC_Time", "UTC Time", "Date(UTC)", "Date", "Time", "Timestamp", "Create Time");
    ...
    const type = pair && side ? (side.toLowerCase() === "sell" ? "sell" : "buy") : classify(rawType, get("Direction", "Cash Flow", "In/Out"), description);
    ...
    // Binance transaction-history exports expose Change as a signed balance delta. Keep its sign.
    if (change !== null && !pair) {
      baseAmount = signedChange(change);
    } else if (pair && amount !== null && total !== null && (type === "buy" || type === "sell")) {
      ...
      baseAmount = signed(amount, type, get("Direction", "Cash Flow", "In/Out"));
    }

    const parser = pair ? "binance-trades" : "binance-account-log";
    const movement = make("binance", parser, row, {
      occurredAt: asDate(utc) || "",
      transactionType: type,
      originalType: rawType,
      direction: get("Direction", "Cash Flow", "In/Out"),
      baseAsset,
      baseAmount,
      quoteAsset,
      quoteAmount,
      ...
      feeAmount: fee === null ? null : -Math.abs(fee),
      price,
      priceCurrency: asAsset(get("Price Currency", "Quote Currency", "Currency")) || assets.quote,
    });
    movement.externalId = stableId("binance", parser, row, get("Transaction ID", "TransactionID", "TXID", "TxID", "Trade ID", "TradeId", "Order ID", "OrderId", "ID", "Hash"));
    movement.raw.sourceFile = file;
    return movement;
  });
}
...
function coinbase(rows: CsvRow[], file: string) {
  return rows.map((row) => {
    ...
    const movement = make("coinbase", "coinbase-statement", row, {
      occurredAt: asDate(get("Timestamp", "Date", "Created At", "Time")) || "",
      transactionType: type,
      originalType: original,
      direction,
      baseAsset: asset,
      baseAmount: signed(qty, type, direction),
      quoteAsset: isTrade && qty !== null ? priceCurrency : null,
      // Coinbase's Total is inclusive of fees/spread; do not subtract the same fee twice from the cash balance.
      quoteAmount: isTrade && qty !== null && cash !== null ? (type === "buy" ? -Math.abs(cash) : Math.abs(cash)) : null,
      feeAsset: fee !== null && !isTrade ? priceCurrency : fee !== null && subtotal !== null && total !== null && Math.abs(total - subtotal) > 1e-12 ? priceCurrency : null,
      feeAmount: fee !== null && (!isTrade || Math.abs((total ?? 0) - (subtotal ?? 0)) < 1e-12) ? -Math.abs(fee) : null,
      price,
      priceCurrency,
    });
    movement.externalId = stableId("coinbase", "coinbase-statement", row, get("Transaction ID", "ID", "Reference", "Hash"));
    movement.raw.sourceFile = file;
    return movement;
  });
}
...
function kraken(rows: CsvRow[], file: string) {
  return rows.map((row) => {
    const get = rowMap(row);
    const original = get("Type", "Transaction Type", "Transaction") || "unknown";
    const subtype = get("Subtype", "Sub Type");
    const pair = pairAssets(get("Pair", "Symbol", "Market"), get("Base Asset"), get("Quote Asset"));
    const side = get("Side", "type").toLowerCase();
    ...
    const direction = get("Direction", "Flow");
    const isTrade = original.toLowerCase() === "trade" && Boolean(pair.base && pair.quote);
    const type = isTrade ? (side === "sell" ? "sell" : "buy") : classify(original, direction, subtype);
    let baseAsset = asset;
    let baseAmount: number | null;
    let quoteAsset: string | null = null;
    let quoteAmount: number | null = null;

    // Kraken Ledger Amount is already signed. A Ledger row never contains two assets.
    if (!isTrade && get("Amount", "Vol", "Volume", "Quantity")) {
      baseAmount = signedChange(amount);
    } else if (isTrade && amount !== null && cost !== null) {
      baseAsset = pair.base;
      baseAmount = side === "sell" ? -Math.abs(amount) : Math.abs(amount);
      quoteAsset = pair.quote;
      quoteAmount = side === "sell" ? Math.abs(cost) : -Math.abs(cost);
    } else {
      baseAmount = signed(amount, type, direction);
    }

    const parser = isTrade ? "kraken-trades" : "kraken-ledger";
    const movement = make("kraken", parser, row, {
      occurredAt: asDate(get("Time", "Timestamp", "Date")) || "",
      transactionType: type,
      originalType: subtype ? `${original} / ${subtype}` : original,
      direction,
      baseAsset,
      baseAmount,
      quoteAsset,
      quoteAmount,
      feeAsset: fee !== null ? asset : null,
      feeAmount: fee === null ? null : -Math.abs(fee),
      price: asNumber(get("Price", "Rate")),
...
function cryptoCom(rows: CsvRow[], file: string) {
  return rows.map((row) => {
    const get = rowMap(row);
    const original = get("Transaction Type", "Type", "Transaction", "Activity", "Biz Type", "Business Type") || "unknown";
    const direction = get("Direction", "In/Out", "Flow", "Side");
    const description = get("Description", "Notes", "Remark", "Details");
    const type = classify(original, direction, description);
    const asset = asAsset(get("Currency", "Coin", "Asset", "Crypto Currency"));
    const change = asNumber(get("Change", "Balance Change", "Amount", "Quantity", "Net Amount"));
    const balance = get("Balance", "Wallet Balance", "Available Balance");
    const fee = asNumber(get("Fee", "Fees", "Transaction Fee"));
    const feeAsset = asAsset(get("Fee Currency", "Fee Coin", "Fee Asset"));
    const pair = pairAssets(get("Pair", "Symbol", "Instrument"), get("Base Asset"), get("Quote Asset"));
    const price = asNumber(get("Price", "Execution Price", "Average Price"));
    const total = asNumber(get("Total", "Cost", "Quote Amount"));
    const isTrade = (type === "buy" || type === "sell") && Boolean(pair.base && pair.quote);
    const amount = asNumber(get("Executed Quantity", "Quantity", "Volume", "Amount"));
    let baseAsset = asset || pair.base;
    let baseAmount = change !== null ? signedChange(change) : signed(amount, type, direction);
    let quoteAsset: string | null = null;
    let quoteAmount: number | null = null;
    ...
    const parser = balance ? "crypto-com-wallet-ledger" : isTrade ? "crypto-com-trades" : "crypto-com-wallet";
    const movement = make("crypto.com", parser, row, {
      occurredAt: asDate(get("Timestamp", "Date", "Time", "Created At")) || "",
      transactionType: type,
      originalType: original,
      direction,
      baseAsset,
      ...
      feeAmount: fee === null ? null : -Math.abs(fee),
      price,
      priceCurrency: asAsset(get("Price Currency", "Quote Currency")) || pair.quote,
    });
    movement.externalId = stableId("crypto.com", parser, row, get("Transaction ID", "Transaction Id", "ID", "Reference", "Trade ID", "Order ID", "Hash"));
    movement.raw.sourceFile = file;
    return movement;
  });
}

function kucoin(rows: CsvRow[], file: string) {
  return rows.map((row) => {
    const get = rowMap(row);
    const original = get("BizType", "Biz Type", "Business Type", "Transaction Type", "Type", "Activity") || "unknown";
    const direction = get("Direction", "Flow", "Side", "In/Out");
    const pair = pairAssets(get("Symbol", "Pair", "Market"), get("Base Coin", "Base Asset"), get("Quote Coin", "Quote Asset"));
    const asset = asAsset(get("Currency", "Coin", "Asset")) || pair.base;
    const amount = asNumber(get("Amount", "Change", "Balance Change", "Quantity"));
    const balance = get("Balance", "Available Balance", "Wallet Balance");
    const total = asNumber(get("Total", "Cost", "Funds", "Quote Amount"));
    const price = asNumber(get("Price", "Filled Price", "Average Price"));
    const fee = asNumber(get("Fee", "Fees", "Transaction Fee"));
    const feeAsset = asAsset(get("Fee Currency", "Fee Coin", "Fee Asset"));
    const type = classify(original, direction);
    const isTrade = (type === "buy" || type === "sell") && Boolean(pair.base && pair.quote) && total !== null;
    const movement = make("kucoin", balance ? "kucoin-account-ledger" : isTrade ? "kucoin-trades" : "kucoin-ledger", row, {
      occurredAt: asDate(get("Time", "Timestamp", "Date", "Created At")) || "",
      transactionType: type,
      originalType: original,
      direction,
      baseAsset: isTrade ? pair.base : asset,
      baseAmount: isTrade && amount !== null ? (type === "buy" ? Math.abs(amount) : -Math.abs(amount)) : signedChange(amount) ?? signed(amount, type, direction),
      quoteAsset: isTrade ? pair.quote : null,
      quoteAmount: isTrade ? (type === "buy" ? -Math.abs(total as number) : Math.abs(total as number)) : null,
      feeAsset,
      feeAmount: fee === null ? null : -Math.abs(fee),
      price,
      priceCurrency: asAsset(get("Price Currency", "Quote Currency")) || pair.quote,
    });
    movement.externalId = stableId("kucoin", movement.raw.parser, row, get("Trade ID", "TradeId", "Order ID", "OrderId", "Transaction ID", "ID", "Hash", "Reference"));
    movement.raw.sourceFile = file;
    return movement;
  });
}

function bybit(rows: CsvRow[], file: string) {
  return rows.map((row) => {
    const get = rowMap(row);
    const original = get("Type", "Transaction Type", "Activity") || "unknown";
    const direction = get("Direction", "Side", "Cash Flow");
    const category = get("Category", "Product Category");
    const asset = asAsset(get("Currency", "Coin", "Asset"));
    const change = asNumber(get("Change", "Change Amount", "Balance Change"));
    const cashFlow = asNumber(get("Cash Flow", "Cashflow"));
    const cashBalance = asNumber(get("Cash Balance", "CashBalance", "Balance", "Wallet Balance"));
    const qty = asNumber(get("Qty", "Quantity", "Amount", "Size"));
    const price = asNumber(get("Trade Price", "Price", "Filled Price"));
    const fee = asNumber(get("Fee", "Fees", "Transaction Fee"));
    const feeAsset = asAsset(get("Fee Coin", "Fee Currency", "Fee Asset"));
    const type = classify(original, direction, category);
    // Bybit Transaction Log's Change/Cash Flow fields are balance-flow data; do not invent a direction from Type.
    const baseAmount = change !== null ? signedChange(change) : cashFlow !== null && cashFlow !== 0 ? signedChange(cashFlow) : signed(qty, type, direction);
    const movement = make("bybit", "bybit-transaction-log", row, {
      occurredAt: asDate(get("Transaction Time", "UTC Time", "Timestamp", "Date", "Time")) || "",
      transactionType: type,
      originalType: original,
      direction,
      baseAsset: asset,
      ...
      feeAmount: fee === null ? null : -Math.abs(fee),
      price,
      priceCurrency: asAsset(get("Price Currency", "Quote Currency")),
    });
    movement.externalId = stableId("bybit", "bybit-transaction-log", row, get("Transaction ID", "TransactionId", "Trade ID", "TradeId", "Order ID", "OrderId", "ID", "UID"));
    movement.raw.sourceFile = file;
    if (cashBalance !== null) movement.raw.row["__coinrenta_reported_balance"] = String(cashBalance);
    if (asset) movement.raw.row["__coinrenta_reported_balance_asset"] = asset;
    return movement;
  });
}

function okx(rows: CsvRow[], file: string) {
  return rows.map((row) => {
    const get = rowMap(row);
    const original = get("Trade Type", "Transaction Type", "Type", "Activity") || "unknown";
    const action = get("Action", "Side", "Direction");
    const asset = asAsset(get("Balance Unit", "Currency", "Asset", "Fee Unit"));
    const balanceChange = asNumber(get("Balance Change", "Change", "Cash Flow"));
    const balance = asNumber(get("Balance", "Position Balance", "Wallet Balance"));
    const amount = asNumber(get("Amount", "Quantity", "Size"));
    const price = asNumber(get("Filled Price", "Price", "Average Price"));
    const fee = asNumber(get("Fee", "Fees"));
    const feeAsset = asAsset(get("Fee Unit", "Fee Currency", "Fee Asset"));
    const type = classify(original, action);
    const movement = make("okx", "okx-trading-history", row, {
      occurredAt: asDate(get("Time", "Timestamp", "Date")) || "",
      transactionType: type,
      originalType: original,
      direction: action,
      baseAsset: asset,
      baseAmount: balanceChange !== null ? signedChange(balanceChange) : signed(amount, type, action),
      quoteAsset: null,
      quoteAmount: null,
      feeAsset,
      feeAmount: fee === null ? null : -Math.abs(fee),
      price,
      priceCurrency: asAsset(get("Price Unit", "Price Currency", "Quote Currency")),
    });
    movement.externalId = stableId("okx", "okx-trading-history", row, get("Trade ID", "TradeId", "Order ID", "OrderId", "Transaction ID", "ID"));
    movement.raw.sourceFile = file;
    if (balance !== null) movement.raw.row["__coinrenta_reported_balance"] = String(balance);
    if (asset) movement.raw.row["__coinrenta_reported_balance_asset"] = asset;
    return movement;
  });
}
...
export function normalizeExchangeCsv(exchange: string, rows: CsvRow[], fileName = "") {
  ...
  const movements = code === "bitpanda" ? bitpanda(rows, fileName)
    : code === "binance" ? binance(rows, fileName)
    : code === "coinbase" ? coinbase(rows, fileName)
    : code === "kraken" ? kraken(rows, fileName)
    : code === "crypto.com" || code === "cryptocom" ? cryptoCom(rows, fileName)
    : code === "kucoin" ? kucoin(rows, fileName)
    : code === "bybit" ? bybit(rows, fileName)
    : code === "okx" ? okx(rows, fileName)
    : (() => { throw new Error(`Exchange no soportado: ${exchange}`); })();
  return movements.map((movement) => ({ ...movement, raw: { ...movement.raw, sourceFile: fileName, sourceExchange: code } }));
}

function parseText(text: string) { return readRows(text); }
