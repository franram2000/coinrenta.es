    ...
    try {
      ...
      if (isRegister) {
        const { data, error: signupError } = await supabase.auth.signUp({
          email: email.trim(),
          password,
          options: { data: { display_name: displayName.trim() || null }, emailRedirectTo: `${SITE_URL}/verificar-correo` },
        });
      ...
      <section className="auth-layout">
        <div className="auth-brand-panel">
          ...
          <div className="auth-brand-copy">
            <span className="auth-eyebrow">Gestión cripto para España</span>
            <h1>{isRegister ? "Empieza a controlar tu cartera." : "Vuelve a tenerlo todo bajo control."}</h1>
            <p>{isRegister ? "Centraliza tus exchanges, movimientos y datos fiscales en un único espacio." : "Accede a CoinRenta para consultar tus activos, operaciones y preparación de Renta."}</p>
          </div>
        ...
        <div className="auth-card">
          <div className="auth-card-head">
            <div><span className="auth-card-kicker">CoinRenta</span><h2>{isRegister ? "Crear cuenta" : "Iniciar sesión"}</h2><p>{isRegister ? "Crea tu cuenta para empezar." : "Introduce tus credenciales para continuar."}</p></div>
            <span className="auth-secure" aria-label="Conexión segura">⌁</span>
          </div>
