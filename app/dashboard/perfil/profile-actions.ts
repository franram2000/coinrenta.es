...
async function paddleRequest(path: string, init: RequestInit = {}) {
  ...
  if (!key) throw new Error("No está configurado el acceso de Paddle para cancelar las suscripciones.");
...
async function paddleCancelAllCustomerSubscriptions(customerId: string) {
  ...
  do {
    const params = new URLSearchParams({
      ...
      status: "active,paused,trialing",
      per_page: "200",
    });
    if (after) params.set("after", after);

    const response = await paddleRequest(`/subscriptions?${params.toString()}`);
    const page = Array.isArray(response?.data) ? response.data : [];
    subscriptions.push(...page);
    after = response?.meta?.pagination?.next || null;
  } while (after);
...
export async function deleteOwnAccount(formData: FormData) {
  ...
  if (activeStates.includes(status)) {
    try {
      let customerId = String(profile?.paddle_customer_id || "").trim();

      if (!customerId && profile?.paddle_subscription_id) {
        const subscription = await paddleRequest(`/subscriptions/${encodeURIComponent(String(profile.paddle_subscription_id))}`);
        customerId = String(subscription?.data?.customer_id || subscription?.data?.customerId || "").trim();
      }

      if (customerId) {
        await paddleCancelAllCustomerSubscriptions(customerId);
      } else if (profile?.paddle_subscription_id) {
        await paddleCancelSubscription(String(profile.paddle_subscription_id));
      }
    } catch (error) {
      throw new Error(`No se pudieron cancelar las suscripciones de Paddle. La cuenta no se ha eliminado. ${error instanceof Error ? error.message : ""}`.trim());
    }
  }
