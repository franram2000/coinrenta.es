...
export default function DashboardSidebar({ role }: { role: string }) {
  ...
  const items: readonly NavItem[] = role === "admin" ? [...nav, ["♙", "Usuarios", "/dashboard/usuarios"] as const] : nav;
  ...
  return <>
    <aside className="app-sidebar">
      ...
      <nav className="app-nav" aria-label="Navegación principal">
        {items.map(([icon, label, href]) => <Link className={`app-nav-item ${activeFor(href) ? "active" : ""}`} href={href} key={href}><span>{icon}</span>{label}</Link>)}
      </nav>
    ...
    <aside id="mobile-dashboard-menu" className={`mobile-menu-panel ${open ? "open" : ""}`} aria-label="Menú móvil" aria-hidden={!open}>
      ...
      <nav className="mobile-menu-nav" aria-label="Navegación móvil">{items.map(([icon, label, href]) => <Link className={`app-nav-item ${activeFor(href) ? "active" : ""}`} href={href} key={href} onClick={() => setOpen(false)}><span>{icon}</span>{label}</Link>)}</nav>
