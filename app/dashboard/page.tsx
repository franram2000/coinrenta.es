...
function latestPositions(rows: Snapshot[]) {
  ...
  for (const row of rows) {
    ...
    if (!existing || timestamp > new Date(existing.captured_at).getTime()) {
      result.set(key, { ...row, account: one(row.account), asset: one(row.asset) });
    }
  }
  return result;
}
...
function buildExchangeRows(current: Snapshot[], history: Snapshot[]) {
  ...
  for (const item of current) {
    ...
    const currentRow = rows.get(key) || { key, name: exchange?.name || connection?.label || "Sin exchange", code: exchange?.code || "—", value: 0, assetsCount: 0, assetsList: [], source: item.source === "calculated" || connection?.provider_type === "csv" ? "CSV" : "API", change: null as number | null };
  ...
  for (const item of history) {
    ...
    const connection = one(account?.connection);
    const exchange = one(connection?.exchange);
    const symbol = String(one(item.asset)?.symbol || "").trim();
    if (!Number.isFinite(numericValue) || !Number.isFinite(timestamp) || !symbol || symbol === "-") continue;
    ...
    positions.set(positionKey(item), numericValue);
  ...
  for (const [key, row] of rows) {
    ...
    if (series.length >= 2 && Number.isFinite(series[0]) && Math.abs(series[0]) > 1e-12) row.change = ((row.value - series[0]) / Math.abs(series[0])) * 100;
...
function makeChart(rows: Snapshot[]) {
  ...
  return { hasData: true, line, area: `M 0 220 L ${points.join(" L ")} L 760 220 Z`, max, min, labels, yLabels, caption: `${entries.length} días con valoración disponible` };
}
