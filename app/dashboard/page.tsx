...
const nav = [
  ["▦", "Resumen", "/dashboard"], ["↔", "Exchanges", "/dashboard/exchanges"],
  ["⇧", "Importar CSV", "/dashboard/importar"], ["≋", "Movimientos", "/dashboard/movimientos"],
  ["€", "Fiscalidad", "/dashboard/fiscalidad"], ["▣", "Informes", "/dashboard/informes"],
];
...
export default async function DashboardPage() {
  ...
  for (const row of snapshots || []) {
    ...
    const key = `${account?.name || "cuenta"}:${asset?.symbol || "asset"}`;
    if (!latest.has(key) || new Date(row.captured_at).getTime() > new Date(latest.get(key).captured_at).getTime()) latest.set(key, { ...row, account, asset });
  }
  const exchanges = new Map<string, { name: string; code: string; value: number; assets: number }>();
  for (const item of latest.values()) {
    const ex = item.account?.connection?.exchange; const key = ex?.code || ex?.name || "other";
    const current = exchanges.get(key) || { name: ex?.name || "Sin exchange", code: ex?.code || "—", value: 0, assets: 0 };
    current.value += Number(item.value_eur || 0); current.assets += 1; exchanges.set(key, current);
  }
  const exchangeRows = [...exchanges.values()].sort((a,b) => b.value-a.value);
  const total = exchangeRows.reduce((s,x) => s+x.value,0);
  const chart = makeChart(snapshots || []);
  const name = profile?.display_name || user.email?.split("@")[0] || "usuario";
  const initials = name.slice(0,1).toUpperCase();
  const year = new Date().getFullYear();

  return <div className="app-shell dashboard-pro-shell">
    <aside className="app-sidebar">
      <CoinRentaLogo />
      <nav className="app-nav" aria-label="Panel principal">{nav.map(([icon,label,href],i) => <Link className={`app-nav-item ${i===0?"active":""}`} href={href} key={href}><span>{icon}</span>{label}</Link>)}<Link className="app-nav-item" href="/dashboard/ayuda"><span>?</span>Ayuda</Link><Link className="app-nav-item" href="/dashboard/configuracion"><span>⚙</span>Configuración</Link></nav>
      <div className="sidebar-pro-card"><div className="pro-card-title"><span className="pro-crown">♛</span><strong>Plan Pro</strong><span className="plan-active">Activo</span></div><p>Acceso completo a las funcionalidades. Sin límites.</p><Link className="btn btn-outline-full" href="/dashboard/configuracion">Gestionar suscripción</Link></div>
      <div className="sidebar-bottom"><LogoutButton /></div>
    </aside>
    <main className="app-main">
      <header className="app-topbar dashboard-topbar-modern"><div className="dashboard-search">⌕ <span>Buscar por activo, exchange, transacción...</span></div><div className="topbar-actions"><span className="notification">♧<i /></span><span className="profile-chip">{initials}</span><div className="profile-summary"><strong>{name}</strong><span>Pro</span></div><span className="profile-chevron">⌄</span></div></header>
      <section className="dashboard-content dashboard-modern-content">
        ...
        <div className="dashboard-layout-grid"><div className="dashboard-main-column">
          ...
          <section className="pro-summary-card"><div className="pro-summary-icon">♛</div><div className="pro-summary-heading"><strong>Tu plan actual</strong><span>Pro</span></div><ul><li>Soporte prioritario</li><li>Sin límites de importaciones</li><li>Informes fiscales avanzados</li><li>Acceso a todos los exchanges</li></ul><Link className="btn btn-outline-full" href="/dashboard/configuracion">Gestionar suscripción</Link></section>
          <section className="panel-card quick-card"><div className="panel-head"><h3>Accesos rápidos</h3></div><div className="quick-grid"><Link href="/dashboard/importar">⇧<span>Importar CSV</span></Link><Link href="/dashboard/exchanges">↻<span>Conectar Exchange</span></Link><Link href="/dashboard/informes">▣<span>Ver informes</span></Link><Link href="/dashboard/ayuda">?<span>Ayuda</span></Link></div></section>
          <section className="panel-card recent-card"><div className="panel-head"><h3>Movimientos recientes</h3><Link className="panel-link" href="/dashboard/movimientos">Ver todos</Link></div>{transactions?.length?<div className="recent-list">{transactions.map(tx=><div className="recent-row" key={tx.id}><span className="recent-icon">◆</span><div><strong>{tx.transaction_type}</strong><small>{(tx.accounts as {name?:string}|null)?.name||"Cuenta"} · {(tx.assets as {symbol?:string}|null)?.symbol||"Activo"}</small></div><div className="recent-value"><strong>{tx.quote_amount?`${Number(tx.quote_amount).toLocaleString("es-ES",{maximumFractionDigits:2})} ${tx.price_currency||""}`:"—"}</strong><small>{new Intl.DateTimeFormat("es-ES",{day:"2-digit",month:"short"}).format(new Date(tx.occurred_at))}</small></div></div>)}</div>:<div className="empty-state"><strong>Sin movimientos todavía</strong><p>Cuando haya actividad aparecerá aquí.</p></div>}</section>
        </aside></div>
