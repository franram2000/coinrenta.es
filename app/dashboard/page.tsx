...
export default async function DashboardPage() {
  ...
  const connections: LocalConnection[] = (rows || []).map((row: any) => {
    ...
    return {
      id: String(row.id), label: row.label || null, provider_type: row.provider_type || null, status: row.status || null,
      last_sync_at: row.last_sync_at || null, last_sync_status: row.last_sync_status || null,
      exchange: String(exchange?.code || 'exchange').toLowerCase(), exchangeName: String(exchange?.name || 'Exchange'),
    };
  });

  return <LocalWorkspace userId={user.id} connections={connections} mode="resumen" isPro={profile?.role === 'pro' || profile?.role === 'admin'} displayName={profile?.display_name || null} />;
}
