...
/**
 * Last-mile safety normalization for the fiscal engine.
 * Bitpanda's original CSV field is authoritative for the economic operation,
 * while the normalized type must always be one of the engine's canonical values.
 */
function fiscalizeMovement(movement: LocalMovement): LocalMovement {
  ...
  let type = stored;
  if (/^sell$|^sold$|^venta$/.test(original)) type = 'sell';
  else if (/^buy$|^bought$|^purchase$|^compra$/.test(original)) type = 'buy';
  else if (/^transfer\(stake\)$|^stake$/.test(original)) type = 'transfer_in';
  else if (/^transfer\(unstake\)$|^unstake$/.test(original)) type = 'transfer_out';
  else if (/^deposit$|^receive$|^received$/.test(original)) type = 'deposit';
  else if (/^withdrawal$|^withdraw$/.test(original)) type = 'withdrawal';
...
export default function LocalWorkspace({ userId, connections, mode, isPro, displayName, year = new Date().getFullYear() }: Props) {
    ...
    async function load() {
      ...
      try {
        ...
        for (const connection of connections.filter((item) => item.provider_type === 'csv' && item.status === 'active')) {
          // Always rebuild the derived dataset when opening a fiscal screen.
          // IndexedDB remains a performance cache for other UI uses, but it is never
          // allowed to become the source of truth for tax data.
        ...
        if (!cancelled) setError(cause instanceof Error ? cause.message : 'No se pudieron cargar los datos fiscales.');
  ...
  const movements = useMemo(() => mergeDatasets(datasets).map(fiscalizeMovement), [datasets]);
  const fifoInput = useMemo(() => datasetToFifo({ version: 6, userId, connectionId: 'merged', sourceVersion: datasets.map((item) => item.sourceVersion).join('|'), fetchedAt: new Date().toISOString(), exchange: 'combined', movements }), [datasets, movements, userId]);
  ...
  if (loading) return <><WorkspaceStyles /><main className="dashboard-content workspace-page"><section className="panel-card"><div className="renta-empty">Recalculando datos fiscales…</div></section></main></>;
