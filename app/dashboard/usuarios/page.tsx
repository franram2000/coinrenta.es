...
export default async function UsersPage() {
  ...
  // El listado completo se hace server-side con la clave administrativa, nunca desde el navegador.
    ...
    <div className="app-shell users-page">
      ...
      <main className="app-main users-main">
        <style>{`
          ...
          .users-table-wrap{overflow-x:auto}.users-table{width:100%;border-collapse:collapse;min-width:970px}.users-table th{padding:11px 22px;text-align:left;color:#718096;font-size:8px;font-weight:900;letter-spacing:.1em;text-transform:uppercase;border-bottom:1px solid var(--cr-border);white-space:nowrap}.users-table td{padding:13px 22px;border-bottom:1px solid rgba(255,255,255,.045);vertical-align:middle}.users-table tbody tr:hover{background:rgba(255,255,255,.018)}.users-table tbody tr:last-child td{border-bottom:0}
          ...
          .user-actions{display:flex;align-items:center;justify-content:flex-end;gap:6px}.user-actions form{display:flex;align-items:center;gap:6px}.user-role-select{height:33px;padding:0 9px;border:1px solid var(--cr-border-strong);border-radius:9px;background:var(--cr-surface-2);color:var(--cr-text);font-size:9px;font-weight:750;outline:none}.user-action{height:33px;padding:0 10px;border:1px solid var(--cr-border-strong);border-radius:9px;background:rgba(255,255,255,.025);color:var(--cr-text);font-size:9px;font-weight:800;cursor:pointer;white-space:nowrap;transition:.18s ease}.user-action:hover:not(:disabled){background:rgba(255,255,255,.065);transform:translateY(-1px)}.user-action:disabled{opacity:.45;cursor:not-allowed}.user-action-save{border-color:var(--cr-primary);background:var(--cr-primary);color:#fff}.user-action-save:hover:not(:disabled){background:var(--cr-primary-hover)}.user-action-delete{border-color:rgba(226,92,92,.35);color:#f09b9b}.user-action-delete:hover:not(:disabled){background:rgba(226,92,92,.1)}
          ...
          @media(max-width:1100px){.users-stats{grid-template-columns:repeat(2,minmax(0,1fr))}}
          @media(max-width:760px){.users-header{display:block}.users-admin{width:max-content;margin-top:18px}.users-directory-head{padding:17px}.users-directory-count{display:none}.users-table-wrap{display:none}.users-mobile-list{display:block}.users-mobile-card{padding:16px;border-bottom:1px solid rgba(255,255,255,.05)}.users-mobile-card:last-child{border-bottom:0}.users-mobile-main{display:flex;align-items:center;gap:11px}.users-mobile-info{min-width:0;flex:1}.users-mobile-info strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px}.users-mobile-info small{display:block;margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--cr-text-muted);font-size:9px}.users-mobile-date{color:#8795a9;font-size:9px;white-space:nowrap}.users-mobile-tags{display:flex;align-items:center;gap:7px;margin-top:12px}.users-mobile-actions{margin-top:11px}.users-mobile-actions .user-actions{display:grid;grid-template-columns:1fr 1fr;gap:7px;justify-content:stretch}.users-mobile-actions .user-actions form:last-child{grid-column:1/-1}.users-mobile-actions form,.users-mobile-actions .user-actions>form{display:flex;min-width:0}.users-mobile-actions form>*{flex:1;min-width:0}.users-mobile-actions .user-role-select{width:100%}.users-mobile-actions .user-actions>form:nth-child(1){grid-column:1}.users-mobile-actions .user-actions>form:nth-child(2){grid-column:2}.users-mobile-actions .user-actions>form:nth-child(3){grid-column:1/-1}}
        ...
        <section className="dashboard-content users-content">
          ...
          <section className="users-directory">
            <div className="users-directory-head"><div><h2>Directorio de usuarios</h2><p>Consulta y modifica el plan o elimina cuentas registradas.</p></div><div className="users-directory-count"><b>{active}</b> activos · {inactive} inactivos</div></div>
            {users.length ? <>
              <div className="users-table-wrap"><table className="users-table"><thead><tr><th>Usuario</th><th>Plan / rol</th><th>Estado</th><th>Alta</th><th style={{textAlign:"right"}}>Gestionar</th></tr></thead><tbody>
                {users.map((item) => {
                  ...
                  return <tr key={item.id}>
                    ...
                    <td><span className={`user-role user-role-${rc}`}>{roleLabel(item.role)}</span></td>
                    <td><span className={`user-status ${isActive ? "user-status-active" : "user-status-inactive"}`}>{isActive ? "Activo" : "Inactivo"}</span></td>
                    <td><span className="user-date">{formatDate(item.created_at)}</span></td>
                    <td><UserActions userId={item.id} role={item.role} isActive={isActive} label={label} isSelf={item.id === user.id} /></td>
                  </tr>;
                })}
              </tbody></table></div>
              <div className="users-mobile-list">{users.map((item) => {
                const label = item.display_name || item.email?.split("@")[0] || "Usuario";
                const isActive = item.is_active !== false;
                return <article className="users-mobile-card" key={item.id}>
                  <div className="users-mobile-main"><span className="user-avatar">{initials(label)}</span><div className="users-mobile-info"><strong>{label}</strong><small>{item.email || item.id}</small></div><span className="users-mobile-date">{formatDate(item.created_at)}</span></div>
                  <div className="users-mobile-tags"><span className={`user-role user-role-${roleClass(item.role)}`}>{roleLabel(item.role)}</span><span className={`user-status ${isActive ? "user-status-active" : "user-status-inactive"}`}>{isActive ? "Activo" : "Inactivo"}</span></div>
                  <div className="users-mobile-actions"><UserActions userId={item.id} role={item.role} isActive={isActive} label={label} isSelf={item.id === user.id} /></div>
                </article>;
              })}</div>
