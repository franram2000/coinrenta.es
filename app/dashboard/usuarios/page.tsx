...
function subscriptionClass(plan: string | null) {
  return plan === "pro" ? "pro" : plan === "essential" ? "essential" : "free";
}
...
export default async function UsersPage() {
  ...
  const pro = users.filter((item) => item.subscription_plan === "pro").length;
  const adminName = me?.display_name || user.email?.split("@")[0] || "Administrador";

  return (
    <main className="dashboard-content users-content">
      <style>{`
        .users-content{padding-bottom:52px}.users-header{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;margin-bottom:26px}.users-header-copy{min-width:0}.users-eyebrow{display:flex;align-items:center;gap:8px;color:#6fdad2;font-size:10px;font-weight:900;letter-spacing:.11em;text-transform:uppercase}.users-eyebrow:before{content:"";width:6px;height:6px;border-radius:50%;background:var(--cr-primary);box-shadow:0 0 0 5px rgba(15,167,160,.1)}.users-header h1{margin:9px 0 7px;font-size:clamp(30px,3.3vw,42px);line-height:1;letter-spacing:-.05em}.users-header p{margin:0;color:var(--cr-text-muted);font-size:13px;line-height:1.6}.users-admin{display:flex;align-items:center;gap:10px;padding:7px 12px 7px 7px;border:1px solid var(--cr-border);border-radius:13px;background:var(--cr-surface);white-space:nowrap}.users-admin-avatar{width:32px;height:32px;display:grid;place-items:center;border-radius:10px;background:var(--cr-primary-soft);color:#75ddd5;font-size:10px;font-weight:900}.users-admin strong{display:block;font-size:11px}.users-admin span{display:block;color:var(--cr-text-muted);font-size:9px;margin-top:2px}.users-stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:16px}.users-stat{position:relative;overflow:hidden;min-width:0;padding:18px 19px;border:1px solid var(--cr-border);border-radius:16px;background:var(--cr-surface);box-shadow:var(--cr-shadow-sm)}.users-stat-label{display:block;color:var(--cr-text-muted);font-size:9px;font-weight:850;letter-spacing:.09em;text-transform:uppercase}.users-stat-value{display:block;margin-top:8px;font-size:25px;line-height:1;letter-spacing:-.045em}.users-stat-note{display:block;margin-top:7px;color:#728096;font-size:9px}.users-stat-primary .users-stat-value{color:#6dd9d2}.users-directory{overflow:hidden;border:1px solid var(--cr-border);border-radius:19px;background:var(--cr-surface);box-shadow:var(--cr-shadow-sm)}.users-directory-head{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:20px 22px;border-bottom:1px solid var(--cr-border)}.users-directory-head h2{margin:0;font-size:16px}.users-directory-head p{margin:4px 0 0;color:var(--cr-text-muted);font-size:11px}.users-directory-count{color:#8d9bb0;font-size:10px}.users-directory-count b{color:#69d8d0}.users-table-wrap{overflow-x:auto}.users-table{width:100%;border-collapse:collapse;min-width:1060px}.users-table th{padding:11px 22px;text-align:left;color:#718096;font-size:8px;font-weight:900;letter-spacing:.1em;text-transform:uppercase;border-bottom:1px solid var(--cr-border)}.users-table td{padding:13px 22px;border-bottom:1px solid rgba(255,255,255,.045);vertical-align:middle}.user-cell{display:flex;align-items:center;gap:11px;min-width:235px}.user-avatar{width:38px;height:38px;display:grid;place-items:center;flex:0 0 38px;border:1px solid rgba(15,167,160,.15);border-radius:11px;background:linear-gradient(145deg,#17343b,#10262d);color:#72d9d2;font-size:10px;font-weight:900}.user-meta{min-width:0}.user-name{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px;font-weight:850}.user-email{display:block;max-width:250px;margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--cr-text-muted);font-size:9px}.user-role{display:inline-flex;align-items:center;gap:6px;padding:6px 9px;border-radius:999px;font-size:9px;font-weight:850;white-space:nowrap}.user-role:before{content:"";width:5px;height:5px;border-radius:50%}.user-role-user{color:#a7b2c3;background:rgba(145,160,182,.08)}.user-role-user:before{background:#8794a8}.user-role-admin{color:#aaa9ff;background:rgba(99,102,241,.1)}.user-role-admin:before{background:#7779ed}.user-subscription-free{color:#a7b2c3;background:rgba(145,160,182,.08)}.user-subscription-free:before{background:#8794a8}.user-subscription-essential{color:#d6c58b;background:rgba(208,177,85,.09)}.user-subscription-essential:before{background:#c9aa51}.user-subscription-pro{color:#70d9d1;background:rgba(15,167,160,.09)}.user-subscription-pro:before{background:var(--cr-primary)}.user-status{display:inline-flex;align-items:center;gap:6px;font-size:9px;font-weight:850}.user-status:before{content:"";width:6px;height:6px;border-radius:50%}.user-status-active{color:#6fd7a9}.user-status-active:before{background:var(--cr-success)}.user-status-inactive{color:#e39a9a}.user-status-inactive:before{background:var(--cr-danger)}.user-date{color:#96a4b8;font-size:10px;white-space:nowrap}.user-actions{display:flex;align-items:center;justify-content:flex-end;gap:6px;flex-wrap:wrap}.user-actions form{display:flex;align-items:center;gap:6px}.user-role-select,.user-subscription-select,.user-action{height:33px;border:1px solid var(--cr-border-strong);border-radius:9px;background:var(--cr-surface-2);color:var(--cr-text);font-size:9px;font-weight:750}.user-role-select,.user-subscription-select{padding:0 9px}.user-action{padding:0 10px;cursor:pointer;font-weight:800}.user-action-save{border-color:var(--cr-primary);background:var(--cr-primary);color:#fff}.user-action-delete{border-color:rgba(226,92,92,.35);color:#f09b9b}.user-action:disabled,.user-role-select:disabled,.user-subscription-select:disabled{opacity:.45;cursor:not-allowed}.user-action-message{width:100%;color:var(--cr-text-muted);font-size:9px;text-align:right}.users-empty{padding:58px 22px;text-align:center;color:var(--cr-text-muted)}.users-mobile-list{display:none}.users-mobile-card{padding:16px;border-bottom:1px solid rgba(255,255,255,.05)}.users-mobile-main{display:flex;align-items:center;gap:11px}.users-mobile-info{min-width:0;flex:1}.users-mobile-info strong,.users-mobile-info small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.users-mobile-info strong{font-size:11px}.users-mobile-info small{margin-top:3px;color:var(--cr-text-muted);font-size:9px}.users-mobile-date{color:#8795a9;font-size:9px;white-space:nowrap}.users-mobile-tags{display:flex;gap:7px;margin-top:12px;flex-wrap:wrap}.users-mobile-actions{margin-top:11px}.users-mobile-actions .user-actions{display:grid;grid-template-columns:1fr 1fr;justify-content:stretch}.users-mobile-actions .user-actions form{grid-column:1/-1;display:grid;grid-template-columns:1fr 1fr auto;gap:6px}.users-mobile-actions .user-action-message{grid-column:1/-1;text-align:left}.users-mobile-actions .user-actions>button{min-width:0}.users-mobile-actions .user-actions>form>*{min-width:0}
      ...
      <div className="users-header">
        <div className="users-header-copy"><div className="users-eyebrow">Administración</div><h1>Usuarios</h1><p>Gestiona el rol y la suscripción de cada cuenta. El rol controla los permisos; la suscripción controla las funciones disponibles.</p></div>
