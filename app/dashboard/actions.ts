...
export async function addExchangeConnection(formData: FormData) {
  ...
  const { data: connection, error } = await supabase.from("exchange_connections").insert({ user_id: user.id, exchange_id: exchangeId, label, status: "pending" }).select("id").single();
  if (!error && connection) await supabase.from("accounts").insert({ user_id: user.id, connection_id: connection.id, account_type: "exchange", name: label || "Nueva cuenta", is_active: true });
