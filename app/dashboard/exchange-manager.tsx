...
import { resyncAllExchanges } from "./actions";
import { addBitpandaApiConnection, addBitpandaCsvConnection, syncBitpandaConnection } from "./connection-actions";
...
export default function ExchangeManager({ exchanges }: Props) {
  ...
  function submitCsv(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        await addBitpandaCsvConnection(formData);
        router.refresh();
  ...
  return <>
    <div className="exchange-actions">
      <button className="btn btn-outline exchange-action" type="button" onClick={resync} disabled={isPending}><span className={isPending ? "connection-spinner" : ""}>↻</span>{isPending ? "Trabajando…" : "Resincronizar"}</button>
      <button className="btn btn-primary exchange-action" type="button" onClick={() => { setOpen(true); setSelected(null); setError(null); setMessage(null); }} disabled={isPending}><span>＋</span>Añadir conexión</button>
    </div>
    ...
    {open && <div className="exchange-modal-backdrop" role="presentation" onMouseDown={close}><div className="exchange-modal" role="dialog" aria-modal="true" aria-labelledby="exchange-modal-title" onMouseDown={(event) => event.stopPropagation()}>
